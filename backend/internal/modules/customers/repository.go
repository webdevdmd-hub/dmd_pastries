...
func (r *Repository) Stats(businessID, branchID, customerID string) (*CustomerStatsResponse, error) {
	...
	if err := r.db.Table("sales").
		Select("COALESCE(SUM(total_amount), 0) AS total_sales_amount, COUNT(*) AS total_orders_count, MAX(sold_at) AS last_purchase_at").
	...
	if err := r.db.Table("payment_refunds pr").
		Joins("JOIN sales s ON s.id = pr.sale_id").
	...
	if err := r.db.Table("sale_refunds sr").
		Joins("JOIN sales s ON s.id = sr.sale_id").
	...
	stats.OutstandingBalance = roundCustomerMoney(stats.TotalSalesAmount - stats.TotalPaidAmount)
