...
type BackendSalesChannelPayload = {
  channel_name?: string;
  channel_type?: string;
  requires_external_order_number?: boolean;
  default_payment_method_id?: string | null;
...
function parsePaymentMethod(value: unknown): PaymentMethod {
  ...
  return {
    ...
    showInBakeryOrders: optionalBoolean(method.show_in_bakery_orders, true),
    showInPurchasing: optionalBoolean(method.show_in_purchasing, true),
    showInExpenses: optionalBoolean(method.show_in_expenses, true),
...
function parseSalesChannel(value: unknown): SalesChannel {
  ...
  const channel = value as BackendSalesChannel;

  if (
    typeof channel.requires_external_order_number !== "boolean" ||
    typeof channel.is_default !== "boolean" ||
    !isRecordStatus(channel.status)
  ) {
    throw new Error("Backend sales channel payload is missing required fields.");
  }

  return {
    id: requiredString(channel.id, "Sales channel ID is missing."),
    businessId: requiredString(channel.business_id, "Sales channel business ID is missing."),
    channelName: requiredString(channel.channel_name, "Sales channel name is missing."),
    channelType: requiredString(channel.channel_type, "Sales channel type is missing."),
    requiresExternalOrderNumber: channel.requires_external_order_number,
    defaultPaymentMethodId: optionalNullableString(channel.default_payment_method_id),
    defaultPaymentMethodName: optionalString(channel.default_payment_method_name),
    commissionRate: optionalNumber(channel.commission_rate),
    isDefault: channel.is_default,
    status: channel.status,
    createdAt: requiredString(channel.created_at, "Sales channel created date is missing."),
    updatedAt: requiredString(channel.updated_at, "Sales channel updated date is missing."),
  };
}
...
function toBackendPaymentMethodPayload(
  ...
  return {
    ...
    ...(payload.requiresReference !== undefined
      ...
      : {}),
    ...(payload.showInPos !== undefined ? { show_in_pos: payload.showInPos } : {}),
    ...(payload.showInBakeryOrders !== undefined
      ? { show_in_bakery_orders: payload.showInBakeryOrders }
      : {}),
    ...(payload.showInPurchasing !== undefined
      ? { show_in_purchasing: payload.showInPurchasing }
      : {}),
    ...(payload.showInExpenses !== undefined ? { show_in_expenses: payload.showInExpenses } : {}),
    ...(payload.showInDashboardCollection !== undefined
      ? { show_in_dashboard_collection: payload.showInDashboardCollection }
      : {}),
    ...(payload.defaultPaymentAccountId !== undefined
      ? { default_payment_account_id: payload.defaultPaymentAccountId }
      : {}),
  };
}
...
export async function getSalesChannels(): Promise<SalesChannel[]> {
  const response = await apiRequest<SalesChannel[]>("/api/v1/settings/sales-channels", {
    authMode: "appwrite",
    parse: (data) => parseList(data, parseSalesChannel),
  });

  return response.data;
}

function toBackendSalesChannelPayload(
  payload: SalesChannelPayload | Partial<SalesChannelPayload>,
): BackendSalesChannelPayload {
  return {
    ...(payload.channelName !== undefined ? { channel_name: payload.channelName } : {}),
    ...(payload.channelType !== undefined ? { channel_type: payload.channelType } : {}),
    ...(payload.requiresExternalOrderNumber !== undefined
      ? { requires_external_order_number: payload.requiresExternalOrderNumber }
      : {}),
    ...(payload.defaultPaymentMethodId !== undefined
      ? { default_payment_method_id: payload.defaultPaymentMethodId }
      : {}),
    ...(payload.commissionRate !== undefined ? { commission_rate: payload.commissionRate } : {}),
    ...(payload.isDefault !== undefined ? { is_default: payload.isDefault } : {}),
...
export async function getSalesChannelById(id: string): Promise<SalesChannel> {
  const response = await apiRequest<SalesChannel>(`/api/v1/settings/sales-channels/${id}`, {
    authMode: "appwrite",
    parse: parseSalesChannel,
  });

  return response.data;
}

export async function createSalesChannel(payload: SalesChannelPayload): Promise<SalesChannel> {
  const response = await apiRequest<SalesChannel, BackendSalesChannelPayload>(
    "/api/v1/settings/sales-channels",
    {
      method: "POST",
      authMode: "appwrite",
      body: toBackendSalesChannelPayload(payload),
      parse: parseSalesChannel,
    },
  );
...
export async function updateSalesChannel(
  ...
  const response = await apiRequest<SalesChannel, BackendSalesChannelPayload>(
    `/api/v1/settings/sales-channels/${id}`,
    {
      method: "PATCH",
      authMode: "appwrite",
      body: toBackendSalesChannelPayload(payload),
...
export async function updateSalesChannelStatus(
  ...
  const response = await apiRequest<SalesChannel, UpdateRecordStatusPayload>(
    `/api/v1/settings/sales-channels/${id}/status`,
    {
      method: "PATCH",
      authMode: "appwrite",
      body: payload,
...
export async function setDefaultSalesChannel(id: string): Promise<SalesChannel> {
  const response = await apiRequest<SalesChannel>(`/api/v1/settings/sales-channels/${id}/default`, {
    method: "PATCH",
    authMode: "appwrite",
    parse: parseSalesChannel,
  });
...
export async function deleteSalesChannel(id: string): Promise<void> {
  await apiRequest<void>(`/api/v1/settings/sales-channels/${id}`, {
    method: "DELETE",
    authMode: "appwrite",
    parse: () => undefined,
  });
