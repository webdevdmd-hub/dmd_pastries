...
export function DashboardMetricPanel({
  ...
  return (
    <Card className="border-brand-cappuccino/70 bg-card/90 shadow-soft">
        ...
        {metrics.map((metric) => {
          ...
          return (
            <div
              ...
              key={metric.label}
            >
              <span className="mb-4 inline-flex rounded-2xl bg-card/80 p-3 text-brand-mocha">
                <Icon className="h-5 w-5" aria-hidden="true" />
              </span>
              <p className="text-xs font-semibold text-brand-mocha">{metric.label}</p>
