...
export function POSBarcodeInput({ inputRef, onLookup }: POSBarcodeInputProps): JSX.Element {
  return (
    <label className="relative block min-w-0">
      ...
      <Input
        className="h-10 w-full min-w-0 rounded-md border-border bg-card pl-10 pr-3 text-sm text-foreground shadow-none placeholder:text-foreground-muted focus-visible:ring-2 focus-visible:ring-ring"
        onKeyDown={(event) => {
          if (event.key === "Enter") {
