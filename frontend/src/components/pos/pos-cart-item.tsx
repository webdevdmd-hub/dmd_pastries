    ...
    <div className="rounded-lg border border-border bg-card p-2">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-body font-medium text-foreground">{item.productName}</p>
          {item.variantName ? (
            <p className="truncate text-[0.68rem] text-foreground-muted">{item.variantName}</p>
          ) : null}
        </div>
        <Button
          aria-label={`Remove ${item.productName}`}
          className="h-7 w-7 rounded-md text-danger-text hover:bg-danger-tint"
      ...
      <div className="mt-2 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        <Button
          className="h-7 justify-self-start rounded-md px-2 text-[0.68rem] text-foreground-muted hover:bg-muted"
