...
export const PURCHASE_INVOICE_DETAIL_TABPANEL_ID = "purchase-invoice-detail-tabpanel";

type PurchaseInvoiceDetailViewTabsProps = {
  active: PurchaseInvoiceDetailTabKey;
  invoiceId: string;
  /** Badge count: the lines on the bill. */
  itemsCount?: number | undefined;
  onTabChange: (tab: PurchaseInvoiceDetailTabKey) => void;
};

type Tab = {
  key: PurchaseInvoiceDetailTabKey;
  label: string;
  badge?: number;
};

/**
 * One strip, three tabs, one bill. Same contract as the other detail strips:
 * each tab is a real `<Link>` with a different `?tab=`, a plain left click
 * swaps the panel in place, arrow keys move focus, Enter activates. Payments
 * fires its own query, hence manual activation.
 */
export function PurchaseInvoiceDetailViewTabs({
  active,
  invoiceId,
  itemsCount,
  onTabChange,
}: PurchaseInvoiceDetailViewTabsProps): JSX.Element {
  ...
  return (
    ...
    >
      {tabs.map((tab, index) => {
        ...
        return (
          <Link
            aria-controls={PURCHASE_INVOICE_DETAIL_TABPANEL_ID}
            aria-selected={selected}
            className={segmentItemClass(selected)}
            data-tab=""
            href={purchaseInvoiceDetailTabHref(invoiceId, tab.key)}
            key={tab.key}
