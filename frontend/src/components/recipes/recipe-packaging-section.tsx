...
function draftLineCost(
  ...
  return {
    totalCost: line.quantityRequired * unitCost,
    unitCost,
  };
}
...
export function RecipePackagingSection({
  ...
  const deleteLine = async (line: RecipePackagingLine): Promise<void> => {
    ...
    const lineName = line.componentProductName ?? line.packagingNameSnapshot;
    if (!(await confirm(recipeLineDeleteConfirmation("packaging", lineName)))) return;
