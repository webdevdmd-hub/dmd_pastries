...
import { CurrentStockTable } from "@/components/reports/inventory/current-stock-table";
...
import { useCurrentStockReport } from "@/hooks/use-inventory-reports";
import { usePermission } from "@/hooks/use-permission";
import { useReportBranches } from "@/hooks/use-reports";
import { getErrorMessage } from "@/lib/api/client";
import { isReportFiltered } from "@/lib/reports/is-report-filtered";

export function CurrentStockPageClient(): JSX.Element {
  ...
  const reportQuery = useCurrentStockReport(filters, canView && hasScope);
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6">
      <ReportPageHeader
        title="Current Stock"
        description="Review current, reserved, available, and reorder stock position."
      />
      <InventoryReportFilterBar
        ...
        <CardContent className="p-5">
          {reportQuery.data && reportQuery.data.length > 0 ? (
            ...
            <InventoryReportEmptyState
              ...
              message="No current stock rows in this period."
              noun="current stock rows"
