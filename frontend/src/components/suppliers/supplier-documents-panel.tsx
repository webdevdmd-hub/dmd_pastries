...
export function SupplierDocumentsPanel({
  ...
  const orders = orderDocuments(ordersQuery.data?.items ?? []);
  const bills = invoiceDocuments(invoicesQuery.data?.items ?? []);
  const credits = returnDocuments(returnsQuery.data?.items ?? []);
  const payments = paymentDocuments(paymentsQuery.data?.items ?? []);
