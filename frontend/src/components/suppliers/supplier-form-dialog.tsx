...
function defaultValues(supplier: Supplier | null): CreateSupplierFormValues {
  return {
    ...
    supplierCategoryId: supplier?.supplierCategoryId ?? null,
...
export function SupplierFormDialog({
  ...
  const submit = async (values: CreateSupplierFormValues): Promise<void> => {
    const payload: CreateSupplierPayload = {
      ...
      supplierCategoryId: values.supplierCategoryId ?? null,
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => (!nextOpen ? onClose() : undefined)}>
      <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Manage supplier contact details, category, address, tax identity, and internal notes.
          </DialogDescription>
        </DialogHeader>
          ...
          <section className="grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-2">
              ...
              {fieldError("supplierName") ? (
                <span className="text-sm text-red-700">{fieldError("supplierName")}</span>
              ) : null}
            </label>
            <label className="grid gap-2">
              ...
              <Select
                onValueChange={(value) =>
                  form.setValue("supplierCategoryId", value === "none" ? null : value)
                }
                value={form.watch("supplierCategoryId") ?? "none"}
              >
                <SelectTrigger>
            ...
            <label className="grid gap-2">
              ...
              />
              {fieldError("website") ? (
                <span className="text-sm text-red-700">{fieldError("website")}</span>
              ) : null}
            </label>
            <label className="grid gap-2">
