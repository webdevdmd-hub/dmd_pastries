...
type BackendOverview = {
  ...
  supplier_categories_count?: number;
...
function parseOverview(value: unknown): MasterDataOverview {
  ...
  return {
    ...
    supplierCategoriesCount: requiredNumber(
      overview.supplier_categories_count,
      "Supplier categories count is missing.",
    ),
...
const collectionPaths: Record<MasterDataCollection, string> = {
  ...
  "supplier-categories": "/api/v1/master-data/supplier-categories",
...
export async function getSimpleCategories(
  collection: Extract<
    MasterDataCollection,
    "ingredient-categories" | "packaging-categories" | "supplier-categories"
  >,
