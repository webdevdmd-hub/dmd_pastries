...
function parseCategory(value: unknown): SupplierCategory {
  if (!isObject(value)) {
    throw new Error("Backend supplier category payload is invalid.");
  }
  ...
  return {
    ...
    categoryName: stringValue(value.category_name, "Supplier category"),
...
function parseSupplier(value: unknown): Supplier {
  ...
  return {
    ...
    supplierCategoryId: optionalString(value.supplier_category_id),
    supplierCategoryName: optionalString(value.supplier_category_name),
    phone: optionalString(value.phone),
    email: optionalString(value.email),
...
function supplierPayload(
  ...
  if (payload.supplierCategoryId !== undefined) {
    nextPayload.supplier_category_id = payload.supplierCategoryId;
  }
...
export async function getSuppliers(params: SupplierFilters): Promise<Supplier[]> {
  const response = await apiRequest<Supplier[]>(
    `/api/v1/suppliers${toQueryString({
      ...
      supplier_category_id: params.supplierCategoryId,
...
export async function getSupplierCategories(): Promise<SupplierCategory[]> {
  const response = await apiRequest<SupplierCategory[]>("/api/v1/master-data/supplier-categories", {
    authMode: "appwrite",
